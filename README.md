...
🔐 强制安全登录：采用会话级验证机制。关闭浏览器或标签页即自动退出登录，每次进入后台均需重新验证密码，极大提升安全性。
...
特别注意：后续修改内容要在 GitHub 上的 _worker.js 进行修改，之后会自动同步到 Pages。
点击 开始设置，然后 保存并部署。
...
上传包含 _worker.js 的 Zip 压缩包 或 文件夹。
...
保存并部署。
💾 兼容模式：绑定 KV (可选)
如果您不想配置 D1，或者使用的是 Snippets 版本，请使用 KV。代码会自动识别。
在 Cloudflare 左侧菜单选择 Workers & Pages -> KV。
点击 创建命名空间 (Create a Namespace)，命名为 BLACKLIST（或任意名称）。
...
🌍 节点来源配置 (Node Sources)
变量名	说明	格式说明
ADD	本地优选 IP 列表	支持换行、逗号分隔。<br>例：1.1.1.1:443#美国, 2.2.2.2#香港
...
自动拉黑机制：默认策略为 5次。同一个 IP 在短时间内刷新网页或订阅超过 5 次，将自动加入数据库/KV黑名单。
